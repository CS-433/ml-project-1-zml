...
if __name__ == "__main__":
    ...
    lambdas = [0, 0.0001, 0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0]
    degrees = [i for i in range(2, 13)]

    traning_groups = utils.group_by_categories(X, column=22) 
    for i, idx in enumerate(traning_groups[1:]):
        print(f"Group", i)
        x_i, y_i = X[idx], y[idx]
        ...
        for degree in degrees:
            for lambda_ in lambdas:
                ...
                for train, test in utils.kfolds(y_i.shape[0], kfold=kfold):
                    ...
                    rmse_test = np.sqrt(2 * utils.mean_square_error(y_test, x_test, weights))
                    rmse += rmse_test
                ...
                accuracy /= kfold
                f1 /= kfold
                print(f"For degree={degree} and lambda={lambda_}: rmse={rmse}, score={accuracy}, f1={f1}")
