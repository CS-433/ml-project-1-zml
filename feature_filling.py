...
def calculate_feature_weight(X):
    ...
    Columns_With_Missing_Features = np.setdiff1d(np.arange(X.shape[1]), Columns_With_All_Features)
...
def fill_features_with_linear_regression(X, Columns_With_Missing_Features = None, Feature_Weights = None):
    if(Columns_With_Missing_Features is None or Feature_Weights is None):
        Columns_With_Missing_Features, Feature_Weights = calculate_feature_weight(X)

    Columns_With_All_Features = np.setdiff1d(np.arange(X.shape[1]), Columns_With_Missing_Features)

    for Missing_Feature_Index in Columns_With_Missing_Features:

        Rows_With_Feature_Index = np.nonzero(~np.isnan(X[:, Missing_Feature_Index]))[0]
        Rows_Without_Feature_Index = np.setdiff1d(np.arange(X.shape[0]), Rows_With_Feature_Index)
        weights = np.array(Feature_Weights[Missing_Feature_Index]).T
        X[np.ix_(Rows_Without_Feature_Index, [Missing_Feature_Index])] = X[np.ix_(Rows_Without_Feature_Index, Columns_With_All_Features)] @ weights
...
def calculate_feature_medians(X):
    ...
    Feature_Median = np.nanmedian(X[np.ix_(np.arange(X.shape[0]), Columns_With_Missing_Features)], axis=1)
...
def fill_features_with_median(X, Columns_With_Missing_Features = None, Feature_Median = None):
    if(Columns_With_Missing_Features is None or Feature_Median is None):
        Columns_With_Missing_Features, Feature_Median = calculate_feature_medians(X)

    for Missing_Feature_Index in Columns_With_Missing_Features:
        Rows_Without_Feature_Index = np.nonzero(np.isnan(X[:, Missing_Feature_Index]))[0]
        X[np.ix_(Rows_Without_Feature_Index, [Missing_Feature_Index])] = Feature_Median[Missing_Feature_Index]
